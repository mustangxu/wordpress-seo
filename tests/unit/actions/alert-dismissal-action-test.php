...
class Alert_Dismissal_Action_Test extends TestCase {
	...
	public function test_dismiss_alert() {
		...
		$this->user
			...
			->with( 1, Alert_Dismissal_Action::USER_META_KEY, true )
			->once()
			->andReturn( [ 'other alert' => true, 'test' => true ] );
	...
	public function test_reset_alert() {
		...
		$this->user
			...
			->with( 1, Alert_Dismissal_Action::USER_META_KEY, true )
			->once()
			->andReturn( [ 'some alert' => true, 'test' => true, 'other alert' => true ] );
		...
		$this->user
			->expects( 'update_meta' )
			->with( 1, Alert_Dismissal_Action::USER_META_KEY, [ 'some alert' => true, 'other alert' => true ] )
			->once()
			->andReturn( [ 'some alert' => true, 'other alert' => true ] );
